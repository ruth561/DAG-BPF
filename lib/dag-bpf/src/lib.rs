...
{
	let dag_task_id = dag_task.node_to_reactor[0] as u32;

	let msg = DagBpfMsg::new_task(dag_task_id, 0).as_bytes();
	urb.send_bytes(&msg).unwrap();

	for i in 1..dag_task.nr_nodes {
		let msg = DagBpfMsg::add_node(dag_task_id, dag_task.node_to_reactor[i] as u32, 0).as_bytes();
		urb.send_bytes(&msg).unwrap();
	}

	for i in 0..dag_task.nr_nodes {
		for j in &dag_task.edges[i] {
			let from_tid = dag_task.node_to_reactor[i] as u32;
			let to_tid = dag_task.node_to_reactor[*j] as u32;
